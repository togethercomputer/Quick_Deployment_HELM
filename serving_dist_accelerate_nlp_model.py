...
class HuggingFaceDistNLPModelInference(FastInferenceInterface):
    def __init__(self, model_name: str, args = None) -> None:
        ...
        model, tokenizer = get_dist_accelerate_tokenizer_model(args['hf_model_name'], args['model_path'])
        self.model = model
        self.tokenizer = tokenizer
        torch.manual_seed(0)
        torch.cuda.empty_cache()
        print(f"<HuggingFaceDistNLPModelInference.__init__> initialization done")

    def dispatch_request(self, args, env) -> Dict:
        print(f"<HuggingFaceDistNLPModelInference.dispatch_request> starts")
        args = args[0]
        ...
        if len(self.task_info["prompt_seqs"][0]) == 0 or self.task_info["output_len"] == 0:
            ...
            print(f"<HuggingFaceDistNLPModelInference.dispatch_request> (empty input or output) return: {result}")
            return result
        else:
            result = self._run_inference()
            torch.cuda.empty_cache()
            print(f"<HuggingFaceDistNLPModelInference.dispatch_request> return: {result}")
            return result

    def _run_inference(self):
        print(f"<HuggingFaceDistNLPModelInference._run_inference> start.")
        ...
        print(f"[INFO] HuggingFaceDistNLPModelInference time costs: {time_elapsed} ms. ")
...
if __name__ == "__main__":
    ...
    fip = HuggingFaceDistNLPModelInference(model_name=args.together_model_name, args={
