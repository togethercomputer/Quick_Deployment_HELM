...
def get_local_huggingface_tokenizer_model(
    ...
    if model_name.startswith("Salesforce/codegen"):
        ...
        if model_path is not None:
            print(
                f"<get_local_huggingface_tokenizer_model> Load from path: {model_path}"
            )
        ...
        model = AutoModelForCausalLM.from_pretrained(
            "facebook/opt-350m", torch_dtype=(dtype if dtype else torch.float16)
        )
        ...
        if model_path is not None:
            ...
            model = T5ForConditionalGeneration.from_pretrained(
                "google/flan-t5-xxl", torch_dtype=torch.bfloat16
            )
    elif model_name == "facebook/opt-iml-30b":
        tokenizer = AutoTokenizer.from_pretrained(
            "facebook/opt-iml-30b", use_fast=False
        )
        if model_path is not None:
            print(
                f"<get_local_huggingface_tokenizer_model> Load from path: {model_path}"
            ...
            model = AutoModelForCausalLM.from_pretrained(
                "facebook/opt-iml-30b", torch_dtype=torch.float16
            )
        ...
        model = T5ForConditionalGeneration.from_pretrained(
            "t5-11b", torch_dtype=torch.bfloat16
        )
    ...
    # Together Computer Models
        ...
        model = AutoModelForCausalLM.from_pretrained(
            "togethercomputer/GPT-JT-X-6B-v1.1"
        )
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            "openlm-research/open_llama_7b_preview_200bt"
        )
        model = AutoModelForCausalLM.from_pretrained(
            "openlm-research/open_llama_7b_preview_200bt"
        )
        ...
        logger.warning(
            "model_path is not None, but model_name is not given. Load from model_path only"
        )
        tokenizer = AutoTokenizer.from_pretrained(
            model_path,
...
def get_local_huggingface_tokenizer_model_llm_int8(
    model_name, model_path=None, dtype=None, auth_token=None
):
    if model_path is None:
        model_path = model_name
    tokenizer = AutoTokenizer.from_pretrained(model_path, use_auth_token=auth_token)
    model = AutoModelForCausalLM.from_pretrained(
        model_path, device_map="auto", load_in_8bit=True, use_auth_token=auth_token
    )
